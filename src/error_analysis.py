...
# Đã bọc thêm ngữ cảnh chuyển đổi kiểu dữ liệu tự động
...
def get_predictions(model, df: pd.DataFrame, tokenizer, device, batch_size: int = 32) -> pd.DataFrame:
    ...
    for batch in loader:
        ...
        # FIX LỖI CHÍ MẠNG: Bọc cấu trúc tính toán forward của DeBERTa bằng ngữ cảnh ép kiểu tự động bfloat16
        ...
        if is_labeled:
            # Tính toán loss cũng cần bọc trong autocast để đồng bộ kiểu dữ liệu
            with autocast(device_type=device.type, dtype=torch.bfloat16):
                losses = ce(logits, labels)
            all_losses.extend(losses.float().cpu().numpy())
