...
# ── Checkpoint helpers ────────────────────────────────────────────────────────
def ckpt_path(run_id: str, tag: str) -> str:
    from configs.config import CFG
    
    # Lấy tên ngắn của model, ví dụ: "microsoft/deberta-v3-base" -> "deberta-v3-base"
    model_slug = CFG.MODEL_NAME.split("/")[-1]
    
    # Tạo thư mục con riêng cho từng model nếu chưa tồn tại
    model_ckpt_dir = os.path.join(CFG.CKPT_DIR, model_slug)
    os.makedirs(model_ckpt_dir, exist_ok=True)
    
    # Trả về đường dẫn hoàn chỉnh nằm trong thư mục của model đó
    return os.path.join(model_ckpt_dir, f"{run_id}_{tag}.pt")
